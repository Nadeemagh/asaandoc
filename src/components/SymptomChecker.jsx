...
Respond in JSON format only, no extra text:
{
  ...
  "advice": "One sentence personalized advice"
    ...
    try {
      ...
      const result2 = await analyze({ symptoms, age, gender, isFollowUp: false });
      const text = result2.data.content?.[0]?.text || "";
  ...
  const askFollowUp = async () => {
    ...
    try {
      const functions = getFunctions(app);
      const analyze = httpsCallable(functions, "analyzeSymptoms");
      const result2 = await analyze({ messages: newHistory, isFollowUp: true });
      const reply = result2.data.content?.[0]?.text || "";
