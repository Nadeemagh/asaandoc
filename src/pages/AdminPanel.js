...
export default function AdminPanel() {
  ...
  const handleDeleteDoctor = async (doctorId, doctorName) => {
    if (!window.confirm(`Delete Dr. ${doctorName}? This cannot be undone.`)) return;
    try {
      await deleteDoctor(doctorId);
      setDoctors(prev => prev.filter(d => d.id !== doctorId));
      showToast(`Dr. ${doctorName} deleted.`);
    } catch {
  ...
  const filteredAppointments = appointments.filter(a => {
    const docMatch = filterDoctor === "All" || a.doctorName?.includes(filterDoctor) || a.doctorId === filterDoctor;
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:T.bg }}>
      <div style={{ textAlign:"center" }}><Spinner /><div style={{ color:T.muted, fontSize:14, marginTop:12 }}>Loading admin data...</div></div>
    </div>
  );
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", fontFamily:"Inter,system-ui,sans-serif", background:T.bg }}>
      ...
      {/* SIDEBAR */}
      <div style={{ width:sidebarOpen?230:64, background:`linear-gradient(180deg,#1a2e3b 0%,#0a1929 100%)`,
        ...
        <div style={{ padding:"18px 14px 14px", borderBottom:"1px solid rgba(255,255,255,0.08)" }}>
          ...
          {sidebarOpen && (
            <div style={{ padding:"8px 10px", background:"rgba(255,165,0,0.15)", borderRadius:8,
        ...
        <div style={{ padding:"24px" }}>
          ...
          {view === "dashboard" && (
            <div>
              <div style={{ marginBottom:22 }}>
                <h2 style={{ margin:"0 0 4px", fontSize:20, fontWeight:800, color:T.text }}>Welcome, Admin 👋</h2>
                <p style={{ margin:0, color:T.muted, fontSize:13 }}>AsaanDoc platform overview</p>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))", gap:12, marginBottom:24 }}>
                <StatCard label="Total Doctors"  value={doctors.length}   icon="👨‍⚕️" color={T.primary} />
                <StatCard label="Total Patients" value={patients.length}  icon="👥"   color={T.accent} />
            ...
            <div>
              ...
              <div style={{ display:"grid", gap:12 }}>
                {doctors.map(doc => {
                  ...
                  const docRevenue = docAppts.filter(a => a.status === "completed")
                    .reduce((s, a) => s + Number(a.clinicFee || 0), 0);
                  return (
                      ...
                      <div style={{ display:"flex", gap:16, alignItems:"flex-start", flexWrap:"wrap" }}>
                        ...
                        {doc.photo ? (
                          <img src={doc.photo} alt={doc.name}
                            style={{ width:60, height:60, borderRadius:"50%", objectFit:"cover", border:`2px solid ${T.border}`, flexShrink:0 }} />
                        ) : (
                          <div style={{ width:60, height:60, borderRadius:"50%", background:doc.color||T.primary,
                        ...
                        <div style={{ flex:1 }}>
                          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:8 }}>
                            <div>
                              ...
                              <div style={{ fontSize:12, color:T.muted }}>⏳ {doc.exp} years experience</div>
                            </div>
                            <div style={{ display:"flex", gap:8 }}>
                              <button onClick={() => setSelectedDoctor(selectedDoctor?.id === doc.id ? null : doc)}
                                style={{ padding:"7px 14px", background:T.primaryLight, color:T.primary,
                                  border:`1.5px solid ${T.primary}`, borderRadius:8, fontSize:12, fontWeight:600, cursor:"pointer" }}>
                                {selectedDoctor?.id === doc.id ? "Hide" : "View Profile"}
                              </button>
                          ...
                          <div style={{ display:"flex", gap:16, marginTop:10, flexWrap:"wrap" }}>
                            ...
                            <div style={{ padding:"8px 14px", background:T.bg, borderRadius:8, textAlign:"center" }}>
                              <div style={{ fontSize:18, fontWeight:800, color:"#16a34a" }}>PKR {docRevenue.toLocaleString()}</div>
                            ...
                            <div style={{ padding:"8px 14px", background:T.bg, borderRadius:8, textAlign:"center" }}>
                              ...
                              <div style={{ fontSize:11, color:T.muted }}>Clinics</div>
                            </div>
                          </div>
                          ...
                          {selectedDoctor?.id === doc.id && (
                            <div style={{ marginTop:16, padding:16, background:T.bg, borderRadius:12 }}>
                              ...
                              {doc.clinics?.map((c, i) => (
                                <div key={i} style={{ padding:"10px 12px", background:T.white, borderRadius:8,
                                  marginBottom:8, borderLeft:`3px solid ${c.isOnline?"#16a34a":T.primary}` }}>
                                  <div style={{ display:"flex", justifyContent:"space-between" }}>
                                    <div style={{ fontWeight:600, fontSize:13, color:T.text }}>
                                      {c.isOnline?"💻":"🏥"} {c.name}
                                    </div>
                                    <div style={{ fontWeight:700, color:c.isOnline?"#16a34a":T.primary }}>
              ...
              {/* Add Doctor Modal */}
              {showAddDoctor && <AddDoctorModal onClose={() => setShowAddDoctor(false)} onSave={async (data) => {
                try {
              ...
              <Card>
                <div style={{ overflowX:"auto" }}>
                  <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                    <thead>
                      <tr style={{ background:`linear-gradient(135deg,${T.primary},${T.primaryDark})` }}>
                        {["#","Name","Email","Phone","Total Appts","Last Visit","Actions"].map(h => (
                    ...
                    <tbody>
                      {filteredPatients.map((p, i) => {
                        const patientAppts = appointments.filter(a => a.patientUid === p.uid || a.patientEmail === p.email);
                        const lastAppt = patientAppts.sort((a,b) => b.date?.localeCompare(a.date))[0];
                        return (
                          <tr key={p.id} style={{ background:i%2===0?T.bg:T.white }}>
                            ...
                            <td style={{ padding:"11px 12px", borderBottom:`1px solid ${T.border}`, fontWeight:700, color:T.primary, textAlign:"center" }}>
                              {patientAppts.length}
                            </td>
                            <td style={{ padding:"11px 12px", borderBottom:`1px solid ${T.border}`, color:T.muted }}>
                              {lastAppt ? formatDate(lastAppt.date) : "—"}
                            </td>
                            <td style={{ padding:"11px 12px", borderBottom:`1px solid ${T.border}` }}>
                              <button onClick={() => {
          ...
          {view === "analytics" && (
            <div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))", gap:12, marginBottom:24 }}>
                <StatCard label="Doctors"    value={doctors.length} icon="👨‍⚕️" color={T.primary} />
              ...
              <Card style={{ marginBottom:16 }}>
                ...
                {doctors.map((doc, i) => {
                  const rev = appointments.filter(a => a.doctorId === doc.id && a.status === "completed")
                    .reduce((s, a) => s + Number(a.clinicFee || 0), 0);
                  const pct = totalRevenue > 0 ? Math.round(rev / totalRevenue * 100) : 0;
                  return (
                    <div key={i} style={{ marginBottom:14 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                        <span style={{ fontSize:13, fontWeight:600, color:T.text }}>{doc.name}</span>
                        <span style={{ fontSize:13, fontWeight:700, color:T.primary }}>PKR {rev.toLocaleString()} ({pct}%)</span>
                      </div>
              ...
              <Card>
                <h3 style={{ margin:"0 0 16px", fontSize:15, fontWeight:700, color:T.text }}>📊 Appointments by Status</h3>
...
function AddDoctorModal({ onClose, onSave }) {
  const [form, setForm] = useState({
    name: "", specialty: "", exp: "", color: "#218EB6", avatar: "",
    qualifications: "", services: "",
    clinics: [{
      name: "", address: "", fee: "", days: [], slots: [],
      startTime: "09:00", endTime: "17:00", isOnline: false
    }]
  });
  ...
  const handleSave = async () => {
    ...
    await onSave({
      ...
      avatar: initials,
      clinics: form.clinics.map(c => ({ ...c, fee: Number(c.fee) || 0 }))
    });
    setSaving(false);
  };
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", zIndex:9999,
      ...
      <div onClick={e => e.stopPropagation()}
        style={{ background:"#fff", borderRadius:16, padding:28, maxWidth:600, width:"100%",
        ...
        <div style={{ marginBottom:14 }}>
          <label style={{ display:"block", fontSize:12, fontWeight:700, color:T.muted,
            textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:6 }}>Profile Color</label>
        ...
        <div style={{ marginBottom:14 }}>
          <label style={{ display:"block", fontSize:12, fontWeight:700, color:T.muted,
            textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:6 }}>Qualifications</label>
          <textarea value={form.qualifications} onChange={e => setForm(f => ({...f, qualifications: e.target.value}))}
            placeholder="MBBS, FCPS..." rows={3}
            style={{ padding:"10px 14px", borderRadius:9, border:`1.5px solid ${T.border}`,
              fontSize:14, color:T.text, width:"100%", outline:"none", fontFamily:"inherit", resize:"vertical" }} />
        </div>

        <div style={{ marginBottom:20 }}>
          <label style={{ display:"block", fontSize:12, fontWeight:700, color:T.muted,
            textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:6 }}>Services (one per line)</label>
        ...
        <div style={{ padding:14, background:T.bg, borderRadius:10, marginBottom:20 }}>
          <div style={{ fontSize:12, color:T.muted, marginBottom:10 }}>
            ℹ️ After adding the doctor, they can set up their clinics, schedule and timings from their own dashboard.
          </div>
        </div>

        <div style={{ display:"flex", gap:10 }}>
          <button onClick={onClose}
            style={{ flex:1, padding:"12px", background:T.white, border:`2px solid ${T.border}`,
