...
const fmtDateLong = (d) => new Date(d + "T00:00:00").toLocaleDateString("en-PK",
  { weekday:"short", year:"numeric", month:"short", day:"numeric" });
...
const generateDailyReportPDF = (appointments, doctor, date) => {
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:30px;padding-bottom:20px;border-bottom:3px solid #218EB6">
    <div><div style="font-size:28px;font-weight:900;color:#218EB6">AsaanDoc</div>
    <div style="font-size:12px;color:#6a8a9a;font-family:serif">صحت کا آسان راستہ</div></div>
    <div style="text-align:right"><div style="font-size:18px;font-weight:800">Daily Appointment Report</div>
    <div style="font-size:13px;color:#6a8a9a">${dateStr}</div>
    <div style="font-size:13px;color:#218EB6;font-weight:600">Dr. ${doctor?.name||"—"}</div>
    <div style="font-size:12px;color:#6a8a9a">${doctor?.specialty||""}</div></div></div>
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:28px">
    ${[["Total",appointments.length,"#218EB6"],["Completed",completed,"#00C897"],["Confirmed",confirmed,"#0369a1"],["Pending",pending,"#F59E0B"]]
      .map(([l,v,c])=>`<div style="padding:16px;border-radius:10px;background:#f8fafc;border-left:4px solid ${c}">
      <div style="font-size:11px;font-weight:700;color:#6a8a9a;text-transform:uppercase">${l}</div>
      <div style="font-size:28px;font-weight:900;color:${c};margin-top:4px">${v}</div></div>`).join("")}
  </div>
  <table style="width:100%;border-collapse:collapse;margin-bottom:24px">
    <thead><tr style="background:linear-gradient(135deg,#218EB6,#155f7a)">
      ${["#","Patient Name","Email","Time","Clinic","Reason","Fee","Status"].map(h=>`<th style="padding:12px;text-align:left;color:#fff;font-size:12px">${h}</th>`).join("")}
  ...
  <div style="border-top:1px solid #e2e8f0;padding-top:16px;display:flex;justify-content:space-between;color:#6a8a9a;font-size:11px">
    <div>Generated by AsaanDoc · asaandoc.com</div>
    <div>Dr. ${doctor?.name||""} · ${doctor?.specialty||""}</div></div>
  </div></body></html>`;
...
const generateInvoicePDF = (appointment, doctor) => {
  ...
  <div style="background:linear-gradient(135deg,#218EB6,#155f7a);padding:30px;border-radius:12px;margin-bottom:30px;display:flex;justify-content:space-between;align-items:center">
    <div><div style="color:#fff;font-size:30px;font-weight:900">AsaanDoc</div>
    <div style="color:rgba(255,255,255,0.7);font-size:13px;font-family:serif">صحت کا آسان راستہ</div>
    <div style="color:rgba(255,255,255,0.8);font-size:12px;margin-top:4px">asaandoc.com</div></div>
    ...
    <div style="color:rgba(255,255,255,0.7);font-size:12px;margin-top:2px">${issueDate}</div></div></div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-bottom:28px">
    <div style="padding:20px;background:#f8fafc;border-radius:10px;border-left:4px solid #218EB6">
      ...
      <div style="font-size:12px;color:#6a8a9a;margin-top:3px">📍 ${appointment.clinicAddress||"—"}</div></div>
    <div style="padding:20px;background:#f8fafc;border-radius:10px;border-left:4px solid #00C897">
      ...
      <div style="font-size:12px;color:#6a8a9a;margin-top:6px">📧 ${appointment.patientEmail||"—"}</div></div></div>
  <table style="width:100%;border-collapse:collapse;margin-bottom:24px">
    <thead><tr style="background:#218EB6">
      ${["Description","Date","Time","Type","Amount"].map(h=>`<th style="padding:12px 16px;text-align:left;color:#fff;font-size:12px">${h}</th>`).join("")}
    </tr></thead>
    <tbody><tr style="background:#f8fafc">
      <td style="padding:14px 16px;border-bottom:1px solid #e2e8f0">
        <div style="font-weight:700">Medical Consultation</div>
        <div style="font-size:12px;color:#6a8a9a;margin-top:3px">${appointment.reason||"General Consultation"}</div></td>
      ...
      <td style="padding:14px 16px;border-bottom:1px solid #e2e8f0;text-align:right;font-weight:800;font-size:15px;color:#218EB6">
        PKR ${Number(appointment.clinicFee||0).toLocaleString()}</td></tr></tbody></table>
  <div style="display:flex;justify-content:flex-end;margin-bottom:30px">
    <div style="min-width:260px">
      <div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid #e2e8f0;font-size:13px">
        <span style="color:#6a8a9a">Subtotal</span><span>PKR ${Number(appointment.clinicFee||0).toLocaleString()}</span></div>
      <div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid #e2e8f0;font-size:13px">
        <span style="color:#6a8a9a">Tax</span><span>PKR 0</span></div>
      <div style="display:flex;justify-content:space-between;padding:14px 16px;margin-top:8px;background:#218EB6;border-radius:8px;color:#fff">
        <span style="font-weight:700;font-size:15px">TOTAL</span>
        <span style="font-weight:900;font-size:18px">PKR ${Number(appointment.clinicFee||0).toLocaleString()}</span></div></div></div>
  <div style="padding:14px 20px;background:#e6faf5;border-radius:10px;border:1.5px solid #00C897;margin-bottom:28px;display:flex;justify-content:space-between;align-items:center">
    <span style="font-weight:700;color:#00C897;font-size:15px">✅ PAYMENT RECEIVED</span>
    <span style="font-size:12px;color:#6a8a9a">${issueDate}</span></div>
  <div style="border-top:1px solid #e2e8f0;padding-top:16px;text-align:center;color:#6a8a9a;font-size:11px">
    ...
    <div>Invoice No: ${invoiceNo} · Generated: ${new Date().toLocaleString("en-PK")}</div></div>
  </div></body></html>`;
  const win = window.open("", "_blank");
  win.document.write(html);
  win.document.close();
...
function ManageSchedule({ doctor, onUpdate, showToast }) {
  ...
  const updateFee = (ci, fee) => {
    const u = JSON.parse(JSON.stringify(clinics));
    u[ci].fee = parseInt(fee)||0;
    setClinics(u);
  };
  const updateTime = (ci, field, val) => {
    const u = JSON.parse(JSON.stringify(clinics));
    u[ci][field] = val;
    setClinics(u);
  };
  const updateAddress = (ci, val) => {
    const u = JSON.parse(JSON.stringify(clinics));
    u[ci].address = val;
    setClinics(u);
  };
    ...
    <div>
      ...
      {activeTab === "profile" && (
        <Card style={{ marginBottom:20 }}>
          ...
          <div style={{ marginBottom:20 }}>
            <label style={{ display:"block", fontSize:12, fontWeight:700, color:T.muted, textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:8 }}>Qualifications & Degrees</label>
          ...
          <div style={{ marginBottom:24 }}>
            <label style={{ display:"block", fontSize:12, fontWeight:700, color:T.muted, textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:8 }}>Services Offered</label>
                ...
                <div>
                  ...
                  <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                    {services.split("\n").filter(s=>s.trim()).map((s,i)=>(
                      <span key={i} style={{ padding:"4px 10px", background:T.primaryLight, color:T.primary, borderRadius:20, fontSize:12, fontWeight:600 }}>{s.trim()}</span>
                    ))}
                  </div>
                </div>
              )}
      ...
      {activeTab === "holidays" && (
        <Card>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr auto", gap:12, marginBottom:20, alignItems:"end" }}>
            ...
            <div>
              <label style={{ display:"block", fontSize:12, fontWeight:700, color:T.muted, textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:8 }}>Reason (optional)</label>
              <input value={newHolidayReason} onChange={e=>setNewHolidayReason(e.target.value)}
                placeholder="e.g. Eid, Conference..."
                style={{ padding:"10px 14px", borderRadius:9, border:`1.5px solid ${T.border}`, fontSize:14, color:T.text, width:"100%", outline:"none", fontFamily:"inherit" }} />
            </div>
            <button onClick={handleAddHoliday} disabled={!newHolidayDate||saving}
              style={{ padding:"10px 20px", background:!newHolidayDate?"#ccc":"#EF4444", color:"#fff",
                border:"none", borderRadius:9, fontWeight:700, fontSize:13, cursor:!newHolidayDate?"not-allowed":"pointer" }}>
              🚫 Mark Off
            </button>
          </div>
          {holidays.length === 0 ? (
              ...
              {holidays.sort((a,b)=>a.date.localeCompare(b.date)).map((h,i) => (
                <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px",
                  ...
                  <button onClick={()=>handleRemoveHoliday(h.date)}
                    style={{ padding:"5px 12px", background:"#fff", color:"#EF4444",
...
export default function DoctorDashboard() {
  ...
  const handleUpdateStatus = async (id, status, appointment) => {
    try {
      ...
      const msgs = { confirmed:"Confirmed ✓", completed:"Marked as completed ✓", cancelled:"Declined." };
    ...
    <div style={{ display:"flex", minHeight:"100vh", fontFamily:"Inter,system-ui,sans-serif", background:T.bg }}>
      ...
      <div style={{ flex:1, overflow:"auto" }}>
        <div style={{ background:T.white,padding:"14px 24px",borderBottom:`1px solid ${T.border}`,
          ...
          <div style={{ display:"flex",alignItems:"center",gap:10 }}>
            {pending.length>0 && (
              <div style={{ padding:"5px 12px",background:"#fffbeb",color:"#F59E0B",borderRadius:20,fontSize:12,fontWeight:700,border:"1.5px solid #F59E0B" }}>
                🔔 {pending.length} Pending
              </div>
            )}
            <button onClick={loadData}
              style={{ padding:"7px 14px",background:T.primaryLight,color:T.primary,border:"none",borderRadius:8,fontSize:12,fontWeight:600,cursor:"pointer" }}>
              🔄 Refresh
            </button>
          </div>
        </div>
          ...
          {view==="dashboard" && (
            <div>
              ...
              <div style={{ display:"flex",justifyContent:"flex-end",gap:12,marginBottom:24 }}>
                <div style={{ padding:"16px 20px",borderRadius:12,background:"linear-gradient(135deg,#16a34a,#15803d)",color:"#fff",boxShadow:"0 4px 14px rgba(22,163,74,0.3)" }}>
                  ...
                  <div style={{ fontSize:26,fontWeight:900 }}>PKR {todayAppts.reduce((s,a)=>s+Number(a.clinicFee||0),0).toLocaleString()}</div>
                  <div style={{ fontSize:11,opacity:0.75,marginTop:4 }}>{todayAppts.length} appointment{todayAppts.length>1?"s":""}</div>
                </div>
                <div style={{ padding:"16px 20px",borderRadius:12,background:"linear-gradient(135deg,#218EB6,#155f7a)",color:"#fff",boxShadow:"0 4px 14px rgba(33,142,182,0.3)" }}>
                  ...
                  <div style={{ fontSize:26,fontWeight:900 }}>PKR {todayAppts.filter(a=>a.status==="completed").reduce((s,a)=>s+Number(a.clinicFee||0),0).toLocaleString()}</div>
                  <div style={{ fontSize:11,opacity:0.75,marginTop:4 }}>{todayAppts.filter(a=>a.status==="completed").length} completed</div>
                </div>
                <div style={{ padding:"16px 20px",borderRadius:12,background:"linear-gradient(135deg,#F59E0B,#d97706)",color:"#fff",boxShadow:"0 4px 14px rgba(245,158,11,0.3)" }}>
                  ...
                  <div style={{ fontSize:26,fontWeight:900 }}>PKR {todayAppts.filter(a=>a.status!=="completed"&&a.status!=="cancelled").reduce((s,a)=>s+Number(a.clinicFee||0),0).toLocaleString()}</div>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20 }}>
                    ...
                    <div style={{ display:"flex",gap:8,alignItems:"center" }}>
                      ...
                      {todayAppts.length>0 && (
                        <button onClick={()=>generateDailyReportPDF(todayAppts,doctor,todayStr)}
                          style={{ padding:"4px 10px",background:T.primaryLight,color:T.primary,border:`1px solid ${T.primary}`,borderRadius:6,fontSize:11,fontWeight:700,cursor:"pointer" }}>
                          📄 PDF
                        </button>
                      )}
                    </div>
                        ...
                        <div key={a.id} style={{ padding:"12px 13px",borderRadius:10,background:T.bg,
                          ...
                          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
                              ...
                              {a.paymentReceipt&&(
                                <div style={{ display:"flex",alignItems:"center",gap:6,marginTop:4 }}>
                                  ...
                                  <button onClick={()=>setReceiptModal(a.paymentReceipt)}
                                    style={{ fontSize:10,color:T.primary,background:"none",border:"none",cursor:"pointer",fontWeight:600,textDecoration:"underline" }}>View</button>
                                </div>
                              )}
                            </div>
                            <div style={{ display:"flex",flexDirection:"column",alignItems:"flex-end",gap:5 }}>
                              ...
                              {a.status==="completed"&&<button onClick={()=>generateInvoicePDF(a,doctor)} style={{ padding:"3px 8px",background:"#f0fdf4",color:"#16a34a",border:"1px solid #16a34a",borderRadius:5,fontSize:10,fontWeight:700,cursor:"pointer" }}>🧾 Invoice</button>}
                            </div>
                          </div>
                        </div>
                      ))}
                ...
                <Card>
                  ...
                  {pending.length===0 ? (
                    <div style={{ textAlign:"center",padding:"28px 0",color:T.muted }}>
                      <div style={{ fontSize:36,marginBottom:8 }}>✅</div><div style={{ fontSize:13 }}>All caught up!</div>
                    </div>
                  ) : (
                    <div style={{ display:"grid",gap:8,maxHeight:340,overflowY:"auto" }}>
                      {pending.map(a=>(
                        <div key={a.id} style={{ padding:"12px 13px",borderRadius:10,background:"#fffbeb",border:"1.5px solid #F59E0B" }}>
                          ...
                          <div style={{ fontSize:11,color:T.muted,marginBottom:2 }}>📅 {formatDate(a.date)} at {formatTime(a.slot)}</div>
              ...
              <Card>
                ...
                {dayAppts.length===0 ? (
                  <div style={{ textAlign:"center",padding:"32px 0",color:T.muted }}>
                    <div style={{ fontSize:32,marginBottom:8 }}>📅</div><div>No appointments on this day</div>
                  </div>
                ) : (
                  <div style={{ display:"grid",gap:8 }}>
                    {dayAppts.sort((a,b)=>a.slot?.localeCompare(b.slot)).map(a=>(
                      <div key={a.id} style={{ display:"flex",alignItems:"center",gap:12,padding:"12px 14px",borderRadius:10,
                        border:`1.5px solid ${a.status!=="cancelled"?T.primary:T.border}`,
                      ...
                      <div style={{ display:"flex",alignItems:"flex-start",gap:14,flexWrap:"wrap" }}>
                        ...
                        <div style={{ flex:1,minWidth:200 }}>
                          ...
                          {a.paymentReceipt ? (
                            ...
                            <div style={{ marginTop:6,fontSize:11,color:T.muted,fontStyle:"italic" }}>⏳ No payment receipt uploaded yet</div>
                          )}
                        </div>
                ...
                <div style={{ display:"flex",alignItems:"center",gap:16,flexWrap:"wrap" }}>
                  <div style={{ flex:1 }}>
                    ...
                    <input type="date" value={reportDate} onChange={e=>setReportDate(e.target.value)}
                      style={{ padding:"10px 14px",borderRadius:9,border:`1.5px solid ${T.border}`,fontSize:14,color:T.text,outline:"none",fontFamily:"inherit" }} />
                  </div>
                        ...
                        <div style={{ marginTop:10 }}>
                          ...
                          <div style={{ display:"flex",flexWrap:"wrap",gap:6 }}>
                            {doctor.services.split("\n").filter(s=>s.trim()).map((s,i)=>(
                              <span key={i} style={{ padding:"4px 10px",background:T.primaryLight,color:T.primary,borderRadius:20,fontSize:11,fontWeight:600 }}>{s.trim()}</span>
                            ))}
                          </div>
                        </div>
                      )}
      ...
      {receiptModal && (
        <div onClick={()=>setReceiptModal(null)}
          style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}>
          <div onClick={e=>e.stopPropagation()}
            style={{ background:"#fff",borderRadius:16,padding:24,maxWidth:640,width:"100%",maxHeight:"88vh",overflow:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.4)" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16 }}>
              <div style={{ fontWeight:700,fontSize:16,color:T.text }}>💳 Patient Payment Receipt</div>
              <button onClick={()=>setReceiptModal(null)} style={{ background:"none",border:"none",fontSize:26,cursor:"pointer",color:T.muted,lineHeight:1 }}>×</button>
            </div>
            ...
            <button onClick={()=>setReceiptModal(null)}
              style={{ marginTop:16,width:"100%",padding:"11px",background:T.primary,color:"#fff",border:"none",borderRadius:10,fontWeight:700,fontSize:14,cursor:"pointer" }}>
              Close
            </button>
          </div>
        </div>
